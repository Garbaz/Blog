...
This means we need some differentation in the syntax between the two. Considering that we usually have many instances of calling a function and few instances of labelling a stream, we want the former be the more convenient one to use. Perhaps something like this:
...
Allow for a function to also take parameters in parenthises instead of as input, for example:
...
__Problem:__ This might not be what we want, since the "first" values of the tupels are now lower in the stack and the "second" values of the tupels are at the top of the stack. In a sense, this could be the inverse of what we want, however, with this interpretation `[(1 11) (2 13) (3 17)] unzip` is equivalent to `[1 2 3] [11 13 17]`, which is clean. But of course unzip could also be defined the other way round if needed.
...
Maybe we can number these danging inputs:
